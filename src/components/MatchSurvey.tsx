...
export function MatchSurvey({ onComplete }: Props) {
  ...
  return (
    <div className="max-w-lg mx-auto">
      {/* Progress bar */}
      <div className="h-1 bg-[#f2f2f2] rounded-full mb-8 overflow-hidden">
        <div
      ...
      <p className="text-sm text-[#6a6a6a] mb-3">{step + 1} / {STEPS.length}</p>
      <h2
        className="text-2xl font-bold text-[#222222] mb-8"
      ...
      <div className="space-y-3">
        {current.options.map((opt) => (
          <button
            ...
            className="w-full text-left px-5 py-4 rounded-xl border border-[#dddddd] text-[#222222] font-medium text-sm hover:border-[#222222] hover:shadow-sm transition-all duration-150"
          >
            {opt.label}
          </button>
        ))}
      ...
      {step > 0 && (
        <button
          className="mt-8 text-sm text-[#6a6a6a] underline underline-offset-2 hover:text-[#222222] transition-colors"
